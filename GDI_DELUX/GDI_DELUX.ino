...
#define greenButtonLED  12
#define redButtonLED 4
...
void setup(void) {
  ...
  // Turn on RED LED (Front Panel) at boot
  statusREDYELLOW();                                            // Red and Yellow Front Panel = SPIFFS Success
  Serial.begin(115200);
  Serial.printf("\n");
  ...
  //WiFi.begin("Simon Ludborzs","Renegade!");
  ...
  if (WiFiType == 0) {
    ...
    flagBlueLED = 1;                                              // Blue LED = Wifi Connected
  } else {
    DEBUG_PRINT("WiFi type is: "); DEBUG_PRINTLN(WiFiType);
