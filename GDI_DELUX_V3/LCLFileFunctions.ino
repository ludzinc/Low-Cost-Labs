...
bool handleFileRead(String path) {
  ...
  if (path.startsWith("/output_")) {
    ...
    Serial.println("Output Request Recieved. Not returning any data (204 response).");
    server.send(204, "text/plain", "");
    return true;                                        // Action completed.  No requirement to serve file: exit.
  }
  String pathWithGz = path + ".gz";
//  Serial.print("Path with gz is: "); Serial.println(pathWithGz);
  String pathIcons = "/icons" + path;
//  Serial.print("Path with Icons is: "); Serial.println(pathIcons);
  if (SPIFFS.exists(pathWithGz) || SPIFFS.exists(pathIcons) || SPIFFS.exists(path)) {
    ...
    if (SPIFFS.exists(pathIcons)) {
      path = "/icons" + path;
//      Serial.print("Icons called with path = "); Serial.println(path);
    }
